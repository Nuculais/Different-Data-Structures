...
namespace BinaryTree
{
    ...
    public class BinarySearchTree<T> where T : IComparable<T>
    {
        ...
        {
			if (_root != null && _root.RightChild != null)
        ...
        //Underlättar visandet av traverseringarna
        public string InSort()
        {
            string Order = "";
            List<T> Sort = Inorder().ToList<T>();
            
            for (int i = 0; i < Sort.Count<T>(); i++)
            {
                Order = Order + Sort[i] + " ";
            }
            return Order;
        ...
        {
            string Order = "";
            List<T> Sort = Postorder().ToList<T>();
